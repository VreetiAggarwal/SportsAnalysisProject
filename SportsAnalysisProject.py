...
fig = px.line(data, x=matches, y="Runs", 
                 title='Runs Scored by the batsman Between 18-Aug-08 - 22-Jan-17')
fig.write_html('first_figure.html', auto_open=True)

 

""" ALL THE MATCHES AT DIFFERENT BATTING POSITIONS """
...
fig1.update_layout(title_text='Number of Matches At Different Batting Positions')
...
""" TOTAL RUNS BY THE BATSMAN At Different Batting Positions """
...
fig2.update_layout(title_text='Runs By the Batsman At Different Batting Positions')
...
""" NUMBER OF CENTURIES IN FIRST INNINGS Vs. Second Innings """
                        
centuries = data.query("Runs >= 100")
fig3 = px.bar(centuries, x=centuries["Inns"], y = centuries["Runs"], 
                color = centuries["Runs"],
                title="Centuries By the Batsman in First Innings Vs. Second Innings")
...
fig4 = px.bar(data, x=data["Opposition"], y = data["Runs"], color = data["Runs"],
            title="Most Runs Against Teams")
fig4.write_html('fifth_figure.html', auto_open=True)

#dataset for all the matches played by the Batsman where strike rate is more than 120
...
fig5 = px.bar(strike_rate, x = strike_rate["Inns"], 
            ...
            title="the Batsman's High Strike Rates in First Innings Vs. Second Innings")
fig5.write_html('sixth_figure.html', auto_open=True)



""" RUNS SCORED AND FOURS PLAYED BY HIM IN EACH INNINGS """
     
fig6 = px.scatter(data_frame = data, x="Runs",
                    y="4s", size="SR", trendline="ols", 
                    title="Relationship Between Runs Scored and Fours")
fig6.write_html('seventh_figure.html', auto_open=True)
